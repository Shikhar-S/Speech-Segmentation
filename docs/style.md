...
from src.metrics import segmentation_evaluator
```
...
- **No relative imports.** Use full package paths:
  `from src.recipe.segmentation import inference`, not `from . import inference`.
...
| Class / Exception | `CapWords` | `SegmentationModel`, `ValueError` |
...
class SegmentationInference:
    """Runs forced-alignment or greedy CTC segmentation.

    Attributes:
        greedy: Whether to use CTC argmax instead of forced alignment.
    """
