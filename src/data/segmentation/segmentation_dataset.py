...
class SegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            phone_timestamps, phones_ipa = self.transform(row)
        phone_pointstamps = [
...
class SegmentationDataModule(L.LightningDataModule):
    def __init__(
        ...
        if transform is None:
            self.transform = transform_for_repo(hf_repo)
...
def build_segmentation_dataset(
    ...
    **kwargs,
    ...
    """Build a SegmentationDataset for one split."""
    ...
    transform = kwargs.get("transform", transform_for_repo(hf_repo))
