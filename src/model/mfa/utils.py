...
def strip_outer_silence_units(
    units: list[SegmentationUnit],
) -> list[SegmentationUnit]:
    """Drop leading/trailing silence units from an MFA prediction."""

    def sil(u: SegmentationUnit) -> bool:
        return u.label is None or is_mfa_silence(u.label)

    i, j = 0, len(units)
    while i < j and sil(units[i]):
        i += 1
    while j > i and sil(units[j - 1]):
        j -= 1
    return units[i:j]
...
# Best effort replacements due to vocab mismatch.
# ``ɖ``/``d̪`` lose place/voicing detail
# and ``aɪ`` is monophthongized; acceptable for boundary placement.
_IPA_TO_MFA_TAMIL: dict[str, str] = {
    "t͡ɕ": "t͡ʃ",  # palatal -> postalveolar affricate
    "d̪": "d",  # no dental d in the model
    "ɖ": "ʈ",  # no voiced retroflex stop (loses voicing)
    "ɽ": "ɾ",  # retroflex flap -> alveolar flap
    "ɨ": "ɪ",  # centralized high vowel -> near-close
    "aɪ": "a",  # no diphthong in the model
    "n̪d̪": "n̪",  # nasal+stop cluster -> single in-model phone (keeps 1:1)
}


def normalize_phones_for_mfa_tamil(phones: list[str]) -> list[str]:
    """Map SSNCE Tamil IPA phones to the tamil_cv acoustic model phone set.
    This will be applied after the per-dataset transform is applied.

    Args:
        phones: Phone labels in the SSNCE IPA convention.

    Returns:
        Phone labels in the tamil_cv phone set.
    """
    return [_IPA_TO_MFA_TAMIL.get(p, p) for p in phones]


_IPA_TO_MFA_THAI: dict[str, str] = {
    "t͡ɕ": "tɕ",  # drop the tie bar (model uses plain digraph)
    "t͡ɕʰ": "tɕʰ",
}


def normalize_phones_for_mfa_thai(phones: list[str]) -> list[str]:
    """Map gTIMIT-Thai tone-marked IPA to the thai_mfa acoustic model phone set.

    Applies after ``process_gtimit_thai_with_tones``. Vowels already carry the
    correct tone diacritic; only ``ə``-vowels (``ə˧``, ``əː˥˩``, …) need the
    base remapped to ``ɤ``, and the affricate tie bars are dropped.

    Args:
        phones: Tone-marked phone labels in the gTIMIT-Thai IPA convention.

    Returns:
        Phone labels in the thai_mfa phone set.
    """
    out: list[str] = []
    for p in phones:
        if p in _IPA_TO_MFA_THAI:
            out.append(_IPA_TO_MFA_THAI[p])
        elif p.startswith("ə"):  # ``ə``/``əː`` + tone -> ``ɤ``/``ɤː`` + tone
            out.append("ɤ" + p[1:])
        else:
            out.append(p)
    return out
