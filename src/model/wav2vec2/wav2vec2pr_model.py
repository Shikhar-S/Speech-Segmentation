...
class Wav2Vec2PRModel(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.frontend = None  # compatibility with model_module.py
        self.error_calculator = ErrorCalculator(
            token_list, kwargs.get("sym_space", "<space>"), sym_blank,
            report_cer=True, report_wer=False
        )

    def forward(self, speech, speech_lengths, text, text_lengths, **kwargs):
        encoder_out, encoder_out_lens = self.encode(speech, speech_lengths)
        loss_ctc, stats = self._calc_ctc_loss(encoder_out, encoder_out_lens, text, text_lengths)
        loss, stats, weight = force_gatherable((loss_ctc, stats, speech.shape[0]), loss_ctc.device)
        return {"loss": loss, "stats": stats, "weight": weight}

    def encode(self, speech: torch.Tensor, speech_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def _calc_ctc_loss(self, encoder_out, encoder_out_lens, ys_pad, ys_pad_lens):
        ...
        if not self.training:
            with torch.no_grad():
                ...
                stats["cer_ctc"] = self.error_calculator(ys_hat.cpu(), ys_pad.cpu(), is_ctc=True)
