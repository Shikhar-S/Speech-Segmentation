...
Usage:
    python -m src.recipe.geolocation.local.prepare_swissgerman_metadata \
      --data_root /path/to/swiss_german \
      --save_path /path/to/metadata.csv
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Prepare Swiss German geolocation metadata CSV"
    )
    parser.add_argument(
        "--data_root",
        type=str,
        default="/work/hdd/bbjs/shared/corpora/swiss_german",
        help="Root directory of the swiss_german corpus (contains SDS200/, STT4SG350v2.1/, post-codes.csv, ...)",
    )
    parser.add_argument(
        "--save_path",
        type=str,
        default="/work/nvme/bbjs/sbharadwaj/powsm/PhoneBench/exp/cache/swissgerman/metadata.csv",
        help="Where to write the combined metadata CSV",
    )
    parser.add_argument(
        "--samples_per_zipcode",
        type=int,
        default=100,
        help="Maximum number of samples per zipcode",
    )
    args = parser.parse_args()

    data_root = args.data_root
    save_path = args.save_path
    samples_per_zipcode = args.samples_per_zipcode
    ###### SDS200
    sds_column_map = {
        "clip_path": "path",
        "duration": "duration",
    ...
    sds_valid_df = read_df_with_location(
        data_root, "SDS200/metadata/splits", "valid", sds_column_map, "SDS200"
    )
    sds_train_df = read_df_with_location(
        data_root, "SDS200/metadata/splits", "train_clean", sds_column_map, "SDS200"
    )
    sds_test_df = read_df_with_location(
        data_root, "SDS200/metadata/splits", "test", sds_column_map, "SDS200"
    )
