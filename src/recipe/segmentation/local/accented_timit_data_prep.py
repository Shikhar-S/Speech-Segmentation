...
Usage:
    python -m src.recipe.segmentation.local.accented_timit_data_prep \\
        --subset L1ENGsimple \\
        --downloads_root /work/nvme/bbjs/sbharadwaj/powsm/xeuspr/exp/downloads \\
        --output_dir     exp/data/global-timit-l1eng-simple-segment
"""
...
def words_text(words_path: Path) -> str:
    ...
    toks = [lab for _, _, lab in intervals if lab.lower() not in _SILENCE_LABELS]
    return " ".join(toks)


def load_subset(
    subset: str, downloads_root: Path
) -> List[GlobalTimitSegment]:
