"""Prepare phonological vectors.
Usage:
    python -m scripts.phonvec_generate_csv --dataset_type timit \
    --dataset_path /work/hdd/bbjs/shared/corpora/TIMIT \
    --output_path /work/nvme/bbjs/sbharadwaj/powsm/xeuspr/exp/data/phonological_vector

    python -m scripts.phonvec_generate_csv --dataset_type voxangeles \
    --dataset_path /work/nvme/bbjs/sbharadwaj/powsm/voxangeles \
    --output_path /work/nvme/bbjs/sbharadwaj/powsm/xeuspr/exp/data/phonological_vector
"""
        ...
        for utterance in tqdm(timit[split]):
            ...
            for phn, start, stop in zip(alignment["utterance"], alignment["start"], alignment["stop"]):
                ...
                if phn in ("b", "d", "g", "p", "t", "k", "jh", "ch"):
                    ...
                    if rows and rows[-1]["audio_path"] == audio_path and rows[-1]["timit_phn"] == closure:
