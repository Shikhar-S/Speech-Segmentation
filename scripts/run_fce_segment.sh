#!/bin/bash
# WavLM-FCE segmentation (SLURM array, 1 task per dataset).
# Distributed segmentation inference + boundary eval.
# FCESegmentationHead.decode emits per-frame argmax boundaries.
#
# Submit:
#   sbatch scripts/run_fce_segment.sh <ckpt_path>
# Resubmit a single failed dataset:
#   sbatch --array=3 scripts/run_fce_segment.sh <ckpt_path>
# Custom subset:
#   sbatch --array=0,4,7 scripts/run_fce_segment.sh <ckpt_path>
#
#SBATCH -A bbjs-delta-gpu
#SBATCH -p gpuA40x4,gpuA100x4
#SBATCH -J fce_segment
#SBATCH -N 1
...
OUT_ROOT="exp/runs/fce_segmentation/${SLURM_ARRAY_JOB_ID:-local}"
ODIR="$OUT_ROOT/$NAME"
mkdir -p "$ODIR"
