...
def main():
    parser = argparse.ArgumentParser(description="Convert JSONL to Kaldi format (wav.scp, text.ctc, text).")
    parser.add_argument("--jsonl", required=True, help="Path to JSONL index file.")
    parser.add_argument("--base_data_path", required=True, help="Prefix for file_name to get wav path.")
    parser.add_argument("--out_dir", required=True, help="Directory to write wav.scp, text.ctc, text, coverage report.")
    parser.add_argument("--lang_tag", default="<eng>", help="Language tag for each utterance (default: <eng>).")
    ...
    with open(args.jsonl) as f:
        for line in f:
            ...
            text_lang_lines.append(f"{utt_id} {args.lang_tag}")
    ...
    print(f"ARPABET→IPA coverage: {total_mapped}/{total} ({coverage_pct:.1f}%) mapped", file=sys.stderr)
    if fallback_counts:
        print(f"Unmapped (fallback) symbols: {sorted(fallback_counts.keys())}", file=sys.stderr)
        print(f"Fallback counts: {dict(sorted(fallback_counts.items(), key=lambda x: -x[1]))}", file=sys.stderr)
