...
Usage::
    ...
    python scripts/data_prep/seg_data_convert.py \\
        --wav_scp     <corpus>/wav.scp \\
        --alignments  <corpus>/alignments      # <utt_id>.TextGrid per utterance
        --output_dir  exp/data/torgo-segment \\
        [--text       <corpus>/text] \\
        [--language   <corpus>/text.lang] \\
        [--tier_name  phones] [--split test] \\
        [--hf_repo    <org>/<repo>] [--public]

Used for TORGO and SSNCE, whose phone TextGrids are MFA alignments corrected
by annotators (not part of the original corpus releases).
...
def main() -> None:
    ...
    p.add_argument(
        "--hf_repo",
        default=None,
        help="Optional Hub repo id (e.g. myorg/torgo-segment) to push the result to.",
    )
    p.add_argument(
        "--public",
        action="store_true",
