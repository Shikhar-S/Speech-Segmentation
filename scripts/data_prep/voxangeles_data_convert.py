...
Usage::
    ...
    python scripts/data_prep/voxangeles_data_convert.py \\
        --wav_scp    $PRISM_DUMP_DIR/test_voxangeles/wav.scp \\
        --text       $PRISM_DUMP_DIR/test_voxangeles/text.good \\
        --va_dir     $VOX_ROOT            # clone of github.com/pacscilab/voxangeles
        --output_dir exp/data/voxangeles-segment \\
        --hf_repo    <org>/<repo>  # then export SEG_REPO_VOXANGELES=<org>/<repo> \\
        [--tier_name phones] [--split test]

The utterance list and audio paths come from the PRiSM Kaldi dump
(``wav.scp`` paths are resolved relative to the dump root); phone TextGrids
come from the VoxAngeles repository.
...
def convert(
    ...
    if hf_repo is not None:
        ddict.push_to_hub(hf_repo, private=private)
