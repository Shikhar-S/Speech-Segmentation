...
if __name__ == "__main__":
    ...
    row = args.rows.split("\n")
    TAKE_COL=[0,1,2,3,4,5,6,7]
    print("Computing weighted average scores... over columns:", [NAMES[i] for i in TAKE_COL])
    print('==='*20)

    for r in row:
        cells = r.split('&')
        if not cells: sys.exit(0)
        ...
        if len(scores) == len(TAKE_COL):
            ...
            print(f"Model: {name} | Score: {w_score:.3f}")
        else:
