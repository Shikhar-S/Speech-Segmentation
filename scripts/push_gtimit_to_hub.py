...
Local DatasetDicts (saved by ``src/recipe/segmentation/local/accented_timit_data_prep.py``)
...
Usage:
    ...
    python scripts/push_gtimit_to_hub.py \\
        --org changelinglab \\
        [--public]                              # else --private (default)
"""
...
def main() -> None:
    ...
    for local, repo_short in REPO_NAMES.items():
        ...
        assert local_path.is_dir(), f"missing local dataset: {local_path}"
        assert readme_path.is_file(), f"missing README.md: {readme_path}"
